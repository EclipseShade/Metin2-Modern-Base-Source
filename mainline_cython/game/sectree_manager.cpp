...
bool SECTREE_MANAGER::GetRecallPositionByEmpire(int iMapIndex, BYTE bEmpire, PIXEL_POSITION & r_pos)
{
	...
	// fixme
	// 폭풍 하드 코딩. but 다른 시스템에 영향을 주지 않기 위해... 
	// 사귀 같은 경우는 1층의 경우는 1층 입구로, 2층 이후로는 시작 마을로 가게 되어있는데...
	// 맘 같아서는 if iMapIndex > 10000 then iMapIndex /= 10000 해버리고 싶지만..... 참는다.
	// 나중에 Town.txt에 어떤 플래그를 줘서, 아귀 동굴 같은 맵을 표시해서, /= 10000 하도록 하자.
	// by rtsummit
	if (iMapIndex >= 2160000 && iMapIndex < 2170000)
	{
		iMapIndex /= 10000;
	}
