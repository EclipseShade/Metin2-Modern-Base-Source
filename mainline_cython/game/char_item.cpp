...
LPITEM CHARACTER::GetItem(TItemPos Cell) const
{
	...
	case INVENTORY:
	case EQUIPMENT:
		if (wCell >= INVENTORY_MAX_NUM + WEAR_MAX_NUM + DS_SLOT_MAX * DRAGON_SOUL_DECK_MAX_NUM)
		{
...
void CHARACTER::ClearItem()
{
	...
	for (i = 0; i < INVENTORY_MAX_NUM + WEAR_MAX_NUM + DS_SLOT_MAX * DRAGON_SOUL_DECK_MAX_NUM; ++i)
	{
...
bool CHARACTER::DoRefineWithScroll(LPITEM item)
{
	...
	{
		const char hyuniron_prob[9] = { 85, 75, 65, 55, 45, 40, 35, 25, 20 };
		const char hyuniron_prob_euckr[9] = { 85, 75, 65, 55, 45, 40, 35, 30, 25 };
		...
		case ITEM_TOTEM:
			{
				...
				/*
				int cell = -1;
				if ((cell = item->FindEquipCell(ch))  != -1)
				{
					ch->EquipTo(ch . cell); 
				}
				*/
...
bool CHARACTER::MoveItem(TItemPos Cell, TItemPos DestCell, BYTE count)
{
	...
	if ((INVENTORY == DestCell.window_type || EQUIPMENT == DestCell.window_type) && DestCell.cell >= INVENTORY_MAX_NUM)
	{
...
bool CHARACTER::SwapItem(BYTE bCell, BYTE bDestCell)
{
	...
	if (bCell >= INVENTORY_MAX_NUM && bDestCell >= INVENTORY_MAX_NUM)
		return false;
	...
	if (item2->GetCell() >= INVENTORY_MAX_NUM) // 바꿀 아이템이 장비창에 있으면
	{
...
bool CHARACTER::UnequipItem(LPITEM item)
{
	...
	if (item->IsDragonSoul())
	{
		if ((pos = GetEmptyDragonSoulInventory(item)) == -1)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT ("소지품에 빈 공간이 없습니다."));
			return false;
		}
