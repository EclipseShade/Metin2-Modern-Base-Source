...
LPITEM CItem::RemoveFromCharacter()
{
	...
	else
	{
		if (GetWindow() != SAFEBOX && GetWindow() != MALL)
		{
			...
			{
				if (m_wCell >= INVENTORY_MAX_NUM) // 아니면 소지품에?
					sys_err("CItem::RemoveFromCharacter: pos >= INVENTORY_MAX_NUM");
				else
				{
					pOwner->SetItem(TItemPos (INVENTORY, m_wCell), NULL);
				}
			}
		}
...
int CItem::FindEquipCell(LPCHARACTER ch, int iCandidateCell)
{
	...
	// 용혼석(ITEM_DS, ITEM_SPECIAL_DS)도  SUB_TYPE으로 구분.
	if ((0 == GetWearFlag() || ITEM_TOTEM == GetType()) && ITEM_COSTUME != GetType() && ITEM_DS != GetType() && ITEM_SPECIAL_DS != GetType())
		return -1;
	...
	{
		long limit = m_pProto->aLimits[i].lValue;
		switch (m_pProto->aLimits[i].bType)
		{
			case LIMIT_LEVEL:
				if (ch->GetLevel() < limit)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("레벨이 낮아 착용할 수 없습니다."));
					return -1;
				}
				break;

			case LIMIT_STR:
				if (ch->GetPoint(POINT_ST) < limit)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("근력이 낮아 착용할 수 없습니다."));
					return -1;
				}
				break;

			case LIMIT_INT:
				if (ch->GetPoint(POINT_IQ) < limit)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("지능이 낮아 착용할 수 없습니다."));
					return -1;
				}
				break;

			case LIMIT_DEX:
				if (ch->GetPoint(POINT_DX) < limit)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("민첩이 낮아 착용할 수 없습니다."));
					return -1;
				}
				break;

			case LIMIT_CON:
				if (ch->GetPoint(POINT_HT) < limit)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("체력이 낮아 착용할 수 없습니다."));
					return -1;
				}
				break;
		}
	...
	{
		if (marriage::CManager::instance().IsMarriageUniqueItem(GetVnum()) && 
			!marriage::CManager::instance().IsMarried(ch->GetPlayerID()))
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("결혼하지 않은 상태에서 예물을 착용할 수 없습니다."));
			return -1;
		}
		...
		{
			// CHECK_UNIQUE_GROUP
			DWORD dwWearUnqiue1 = ch->GetWear(WEAR_UNIQUE1)->GetVnum();
			int iWearUnique1Group = ch->GetWear(WEAR_UNIQUE1)->GetSpecialGroup();
			if (iWearUnique1Group && iWearUnique1Group == GetSpecialGroup() ||
					dwWearUnqiue1 == GetVnum())
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("같은 종류의 유니크 아이템 두 개를 동시에 장착할 수 없습니다."));
				return -1;
			}
		...
		else
		{
			...
			{	    
				DWORD dwWearUnqiue2 = ch->GetWear(WEAR_UNIQUE2)->GetVnum();
				int iWearUnique2Group = ch->GetWear(WEAR_UNIQUE2)->GetSpecialGroup();
				if (iWearUnique2Group && iWearUnique2Group == GetSpecialGroup() ||
						dwWearUnqiue2 == GetVnum())
				{
