...
p4 sync $M2ROOT/dist/$LOCALE/Test/Server/db_$LOCALE_FULLNAME  2> ~/ERROR_LOG.txt
#p4 revert $M2ROOT/dist/$LOCALE/Test/Server/db_$LOCALE_FULLNAME  2> ~/ERROR_LOG.txt
if [ ! $? -eq 0 ]; then
	exit 8
fi
p4 edit $M2ROOT/dist/$LOCALE/Test/Server/db_$LOCALE_FULLNAME  2> ~/ERROR_LOG.txt
if [ ! $? -eq 0 ]; then
	exit 9
fi
mv ../db_r"$P4_VERSION" $M2ROOT/dist/$LOCALE/Test/Server/db_$LOCALE_FULLNAME  2> ~/ERROR_LOG.txt
if [ ! $? -eq 0 ]; then
	p4 revert $M2ROOT/dist/$LOCALE/Test/Server/db_$LOCALE_FULLNAME	
	exit 10
fi
REV_RES=`p4 revert -a $M2ROOT/dist/$LOCALE/Test/Server/db_$LOCALE_FULLNAME 2> ~/ERROR_LOG.txt`
if [ ! $? -eq 0 ]; then
	p4 revert $M2ROOT/dist/$LOCALE/Test/Server/db_$LOCALE_FULLNAME	
	exit 11
fi
...
p4 submit -d $DESCRIPTION $M2ROOT/dist/$LOCALE/Test/Server/db_$LOCALE_FULLNAME  2> ~/ERROR_LOG.txt
if [ ! $? -eq 0 ]; then
	p4 revert $M2ROOT/dist/$LOCALE/Test/Server/db_$LOCALE_FULLNAME	
	exit 13
fi
