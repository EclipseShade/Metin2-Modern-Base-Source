...
if [ "$REVISION" != "head" ]; then
	...
	p4 sync -f ../Extern/...@$REVISION > sync_list.txt 2> ~/ERROR_LOG.txt
else
	p4 sync ...#head > sync_list.txt 2> ~/ERROR_LOG.txt
	p4 sync ../Extern/...#head > sync_list.txt 2> ~/ERROR_LOG.txt
fi
...
P4_VERSION=`p4 changes -m1 $SERVER_PATH/game/src/... | cut -d' ' -f2 2> ~/ERROR_LOG.txt`
...
p4 sync $M2ROOT/dist/$LOCALE/Test/Server/game_$LOCALE_FULLNAME  2> ~/ERROR_LOG.txt
if [ ! $? -eq 0 ]; then
	exit 8
fi
...
if [ ! $? -eq 0 ]; then
	p4 revert $M2ROOT/dist/$LOCALE/Test/Server/game_$LOCALE_FULLNAME	
	exit 10
fi
...
p4 submit -d $DESCRIPTION $M2ROOT/dist/$LOCALE/Test/Server/game_$LOCALE_FULLNAME  2> ~/ERROR_LOG.txt
if [ ! $? -eq 0 ]; then
	p4 revert $M2ROOT/dist/$LOCALE/Test/Server/game_$LOCALE_FULLNAME	
	exit 13
fi
